...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		// === THE BOREDOM WEBSITE — Pulsing + Mouse Kill ===
		
		// Timing (ms)
		const fadeIn = 4000;   // 4s fade in
		const hold   = 8000;   // 8s hold
		const fadeOut= 8000;   // 8s fade out
		const pause  = 11000;  // 11s pause (stillhet)
		const totalCycle = fadeIn + hold + fadeOut + pause; // 31s totalt
		const text2Delay = fadeIn + hold; // 12s forsinkelse
		
		// Idle / wake thresholds (in ticks at 30fps)
		const idleThresholdTicks = 180; // ~6 sekunder
		...
		function startPulses() {
		  ...
		  // text2 loop (starter forskjøvet, men holder like lang total tid)
		  createjs.Tween.get(t2, {loop:true})
		    .wait(text2Delay)
		    .to({alpha:1}, fadeIn)
		    .wait(hold)
		    .to({alpha:0}, fadeOut)
		    // fyll på med nok pause til at summen blir totalCycle:
		    .wait(totalCycle - text2Delay - fadeIn - hold - fadeOut);
		}
		
		function stopPulsesAndHideInstantly() {
		  createjs.Tween.removeTweens(t1);
		  createjs.Tween.removeTweens(t2);
		  createjs.Tween.get(t1).to({alpha:0}, 250).call(()=>{ t1.alpha = 0; });
		  createjs.Tween.get(t2).to({alpha:0}, 250).call(()=>{ t2.alpha = 0; });
		}
		
		// Start pulses on load
		startPulses();
		...
		//stage.on("stagemousemove", () => {
		 // if (!isKilled) {
		 //   isKilled = true;
		 //   stopPulsesAndHideInstantly();
		//  }
		 // idleTicks = 0;
		//});
		
		// Funksjon for å "drepe" teksten (brukes av både mus og tastatur)
		function killText() {
		  if (!isKilled) {
		    isKilled = true;
		    stopPulsesAndHideInstantly();
		...
		// Mouse movement → kill
		stage.on("stagemousemove", killText);
		
		// Keyboard press → kill
		window.addEventListener("keydown", killText);
